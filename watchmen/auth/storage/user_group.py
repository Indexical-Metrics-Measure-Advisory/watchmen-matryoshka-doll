...
def get_user_group(user_group_id, current_user) -> UserGroup:
    return storage_template.find_one({"and": [{"userGroupId": user_group_id}, {"tenantId": current_user.tenantId}]},
                                     UserGroup,
                                     USER_GROUPS)
...
def get_user_group_list_by_ids(user_group_ids: list, current_user) -> List[UserGroup]:
    if user_group_ids:
        return storage_template.find_(
            {"and": [{"userGroupId": {"in": user_group_ids}}, {"tenantId": current_user.tenantId}]}, UserGroup,
            USER_GROUPS)
    else:
        return []
...
def get_user_group_by_name(name, current_user):
    return storage_template.find_one({"and": [{"name": name}, {"tenantId": current_user.tenantId}]}, UserGroup,
                                     USER_GROUPS)
