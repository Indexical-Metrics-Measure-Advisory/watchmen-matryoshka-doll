...
def get_space_by_id(space_id: str, current_user: User) -> Space:
    return storage_template.find_one({"and": [{"spaceId": space_id}, {"tenantId": current_user.tenantId}]}, Space,
                                     SPACES)
...
def get_space_list_by_ids(space_ids, current_user) -> List[Space]:
    if space_ids:
        return storage_template.find_({"and": [{"spaceId": {"in": space_ids}}, {"tenantId": current_user.tenantId}]},
                                      Space, SPACES)
    else:
        return []
...
def get_filters_by_id(space_id, current_user) -> List[SpaceFilter]:
    space: Space = storage_template.find_one({"and": [{"spaceId": space_id}, {"tenantId": current_user.tenantId}]},
                                             Space, SPACES)
...
def load_space_list_by_name(name, current_user) -> List[Space]:
    return storage_template.find_({"and": [{"name": {"like": name}}, {"tenantId": current_user.tenantId}]}, Space,
                                  SPACES)


def load_space_list_by_user_id_with_pagination(group_ids, pagination: Pagination, current_user) -> DataPage:
    return storage_template.page_({"and": [{"groupIds": {"in": group_ids}}, {"tenantId": current_user.tenantId}]},
                                  [("spaceId", "desc")],
                                  pagination, Space, SPACES)
