...
def query_space_with_pagination(query_name: str, pagination: Pagination,current_user) -> DataPage:
    return page_({"and":[{"name": {"like": query_name}},{"tenantId":current_user.tenantId}]}, [("name", "desc")], pagination, Space, SPACES)
...
def load_space_list_by_user_id_with_pagination(group_ids, pagination: Pagination,current_user) -> DataPage:
    return page_({"and":[{"groupIds": {"in": group_ids}},{"tenantId":current_user.tenantId}]}, [("spaceId", "desc")], pagination, Space, SPACES)
