...
def load_console_subject_list_by_ids(subject_id_list, current_user):
    return storage_template.find_({"and": [{"subjectId": {"in": subject_id_list}}, {"tenantId": current_user.tenantId}]},
...
def load_console_subject_by_id(subject_id, current_user) -> ConsoleSpaceSubject:
    return storage_template.find_one({"and": [{"subjectId": subject_id}, {"tenantId": current_user.tenantId}]}, ConsoleSpaceSubject,
...
def rename_console_subject_by_id(subject_id, name):
    return storage_template.update_one_first({"subjectId": subject_id}, {"name": name}, ConsoleSpaceSubject, CONSOLE_SPACE_SUBJECTS)
