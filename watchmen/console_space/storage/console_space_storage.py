...
def update_console_space(console_space: ConsoleSpace):
    return storage_template.update_one_first({"connectId": console_space.connectId}, console_space, ConsoleSpace, CONSOLE_SPACES)
...
def load_console_space_by_id(connect_id: str, current_user) -> ConsoleSpace:
    return storage_template.find_one({"and": [{"connectId": connect_id}, {"tenantId": current_user.tenantId}]}, ConsoleSpace,
                    CONSOLE_SPACES)
...
def load_console_space_list_by_user(user_id: str, current_user: User) -> ConsoleSpace:
    return storage_template.find_({"and": [{"userId": user_id}, {"tenantId": current_user.tenantId}]}, ConsoleSpace, CONSOLE_SPACES)


def load_console_space_template_list_by_user(user_id: str, current_user: User) -> ConsoleSpace:
    return storage_template.find_({"and": [{"userId": user_id}, {"tenantId": current_user.tenantId}, {"isTemplate": True}]},
...
def update_console_space_graph(console_space_graph: ConnectedSpaceGraphics) -> ConnectedSpaceGraphics:
    ...
    '''
    return storage_template.update_one_first({"connectId": console_space_graph.connectId}, console_space_graph, ConnectedSpaceGraphics,
                            "console_space_graph")
...
def load_console_space_graph_by_user_id(user_id: str, current_user) -> List[ConnectedSpaceGraphics]:
    ...
    return storage_template.list_({"and": [{"userId": user_id}, {"tenantId": current_user.tenantId}]}, ConnectedSpaceGraphics,
                 "console_space_graph")
...
def load_console_space_graph(connect_id: str, current_user) -> ConnectedSpaceGraphics:
    ...
    return storage_template.find_one({"and": [{"connectId": connect_id}, {"tenantId": current_user.tenantId}]}, ConnectedSpaceGraphics,
                    "console_space_graph")
