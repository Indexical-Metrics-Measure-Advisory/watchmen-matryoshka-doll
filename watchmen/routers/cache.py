...
@router.get("/cache/clear/all", tags=["admin"], response_model=list)
def clear_all():
    cacheman.clear_all()


@router.get("/cache/delete/topic", tags=["admin"], response_model=list)
def delete_topic_cache(topic_name, current_user: User = Depends(deps.get_current_user)):
    ...
    cacheman[TOPIC_BY_ID].delete(topic_name)


@router.get("/cache/clear/topics", tags=["admin"], response_model=list)
...
@router.get("/cache/delete/pipeline", tags=["admin"], response_model=list)
...
@router.get("/cache/delete/topic/pipelines", tags=["admin"], response_model=list)
...
@router.get("/cache/clear/topics/pipelines", tags=["admin"], response_model=list)
def clear_pipelines_cache(current_user: User = Depends(deps.get_current_user)):
    cacheman[PIPELINES_BY_TOPIC_ID].clear()


@router.get("/cache/delete/columns", tags=["admin"], response_model=list)
...
@router.get("/cache/clear/columns", tags=["admin"], response_model=list)
